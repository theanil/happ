...
function ShowMore(total, id, Offset)
{
	...
	newoffset = Number(Offset) + Number(10); 
	//newoffset = Offset + 10;
...
{	
	...
	success: function (result) {
		if(result.status) 
		{			
			var newtotal = Number(result.Offset) + Number(10);
			//alert(result.S_ID);
			//alert(result.Offset);
			//alert(result.Total);
			//alert(newtotal);
		...
		success: function (result) {
			if(result.status) 
			{
				...
				{									
					...
					if(result[0].bidding_type == 'Not Specified')
					{
					...
					detail_data = detail_data + '<li><a href="#" onclick="EmailtoMe(' + result[0].site_tender_id + ');return false;">' + "E-mail Document" +'</a> </li>';						
				}
				...
				if(ntype == '9') //projects
				{
					...
					detail_data = detail_data + '<li>Owner/Implementing Agency: <p class="wrap">' + result[0].owner_address.replace(/(\r\n|\n\r|\r|\n)/g, "<br>") + '</p></li>';
